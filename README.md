...
- **Field controllers** — a point, line, curve, polyline or image placed on the panel drives one channel across it: hole size, hole spacing, hole rotation, or the superellipse morph. Spacing varies the density of the layout itself — the grid modes and Cross-hatch warp their lattice around each controller, opening or drawing it in both directions, and the disc radius follows it in Scatter and Voronoi, the step in Spiral, Fibonacci and Path, the line separation in Flow Lines — and under Flow Lines the rotation channel becomes the direction field the lines follow. Points, lines and curves are drawn straight onto the canvas; polylines and images are added at the centre and then shaped by their handles. Reach, falloff, strength and one-sidedness per controller, each of which can also be inverted — strongest beyond its reach rather than at its geometry, so a sheet stays nominal at the middle and opens toward its edges — and one controller can follow another's geometry. Everything is edited on the canvas over a live heat-map of the channel: drag a controller by its body or by its handles, Shift locks to 45°, double-click a polyline to add or drop a vertex, Delete removes it
- **Halftone from a picture** — an image controller reads its pixels as a halftone: black pixels set one value, white pixels another, so a photograph becomes a field of hole sizes (drop an image anywhere on the page, press Halftone preset, Fit to sheet). Invert, gamma and levels shape the tone curve, which the inspector graphs; a mask mode reads brightness as strength instead, for composing with other controllers
- **Size gradient** — multi-layer scalar fields (linear / radial / angular / spiral × ramp / peak / wave / noise / steps) scale hole size across the whole panel, edited directly on the canvas with Photoshop-style gizmo handles; includes presets, randomize, undo/redo. Controllers multiply on top of it
- **Path curves** — a pen tool that adds a vertex per click (Shift locks to 45°), curves dragged whole or by their vertices, double-click on the curve to add a vertex there and on a vertex to drop it
- **Sheet & boundary** — panel size, per-side margins, rounded panel corners; the perforation boundary as a rectangle, an ellipse, or any closed outline drawn on the canvas or imported from an SVG file (counters kept); cutouts (circle, rectangle, polygon) as keep-outs, dragged by their body or handles; the sheet optionally trimmed to the boundary, which the export writes as a cut path; hole removal by click
- **Shape editor** — a Pathfinder for hole shapes: stack circles, rectangles, hexagons, stars, triangles, diamonds and polygons, each set to Union, Subtract, Intersect or Exclude, and handle them the way a vector editor does — drag to move, corner grips to resize (Shift keeps the proportions), a knob to rotate (Shift in 15° steps), polygon vertices dragged and added by double-click, its own undo, Delete, Ctrl+D
- **Workspace** — a rail of pages on the left (Project, Pattern, Boundary B, Fields F — the size gradient (G) and the controllers as one list of field layers — Path P, Remove R, Taper, Export), a canvas with a per-mode tool rail and a status bar, and an inspector on the right that shows the open page's controls — the pages that own a canvas mode bring it with them; a command palette on Ctrl+K reaches every command, page, layout and hole shape
