...
- **Workspace** — a mode rail on the left (Select V, Boundary B, Size gradient G, Fields F, Path P, Remove R), a canvas with a per-mode tool rail and a status bar, and a foldable inspector on the right; a command palette on Ctrl+K reaches every command, layout and hole shape
