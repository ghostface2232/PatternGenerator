...
- **A constant ligament without a lattice** — Voronoi and Flow Lines place their holes so that exactly the edge gap of metal is left between any two neighbours, everywhere, however irregular the pattern looks. In Voronoi the hole is the cell polygon and in Flow Lines it is a slot; in both the hole-shape dropdown steps aside and the panel says so. Widening the holes afterwards with a size controller eats into that margin like it does in any other mode, and the ligament readout follows it down
