...
- **A constant ligament without a lattice** — Voronoi and Flow Lines both leave exactly the edge gap of metal between any two neighbours, everywhere, however irregular the pattern looks. In Voronoi the hole is the cell polygon and in Flow Lines it is a slot that narrows and widens along its own length; in both the hole-shape dropdown steps aside and the panel says so
