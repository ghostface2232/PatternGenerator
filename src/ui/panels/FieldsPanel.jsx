...
import { Circle, Contrast, Image as ImageIcon, Lock, Maximize2, Minus, Spline, SquarePen, Trash2, Upload, Waves, Waypoints, X } from "lucide-react"; // prettier-ignore
import { MAX_VARIATION_LAYERS, MORPH_SHAPE } from "../../core/constants.js";
import {
  CHANNEL_INFO,
  EDITABLE_CHANNELS,
  FALLOFFS,
...
import { GradientInspector } from "./GradientInspector.jsx";
...
// The Fields page: everything that varies the holes across the sheet, as ONE
// list of field layers per channel. On the size channel the list starts with
// the gradient layers — global scalar fields (space × profile) that scale every
// hole — followed by the controllers: local geometry (a point, a line, a
// curve, a polyline, a picture) that pulls the channel toward a target where it
// reaches. The two used to be two panels and two canvas modes for what is one
// question, "how does size vary here?"; the gradient and the controllers
// multiply, so they belong in one stack, the way a gradient fill and a
// brush-masked adjustment share one Layers panel. Selecting a row puts its
// handles on the canvas — the gradient's gizmo or the controller's handles —
// in the one Fields mode; the inspector below is the selected row's.
export function FieldsPanel() {
  const { doc, theme, ui, actions, selectedController: selected, selectedVariationLayer } = useEditor();
  const { dark } = theme;
  const { fields, assets, variation } = doc;
  const { fieldEditMode, activeChannel, selectedControllerId, gradientSelected } = ui;
  ...
  const onSize = activeChannel === "size";
  // The gradient rows exist while the block is on; "add" switches it on and the
  // last removal switches it off, so the list is the block.
  const gradientLayers = onSize && variation.enabled ? variation.layers : [];
  const gradientFull = variation.enabled && variation.layers.length >= MAX_VARIATION_LAYERS;
  // The page's one switch reads on while anything on it is live.
  const enabled = fields.enabled || variation.enabled;
  ...
  const layerCount = gradientLayers.length + channelControllers.length;
  ...
  return (
    <Section
      ...
      title="Fields"
      theme={theme}
      right={<Toggle value={enabled} onChange={actions.setFieldsEnabled} dark={dark} label="Fields" />}
    >
      <button
        onClick={actions.toggleFieldEditMode}
        aria-label="Edit fields on the canvas"
      ...
      >
        {EDITABLE_CHANNELS.map(channel => {
          ...
          const count =
            fields.controllers.filter(c => c.channel === channel).length +
            (channel === "size" && variation.enabled ? variation.layers.length : 0);
          return (
            <button
              key={channel}
      ...
      {/* The add row: on the size channel the gradient comes first, since it is
          the broad brush; every channel then offers the controller kinds. */}
      <div style={groupLabel}>
        Add
        <span style={{ marginLeft: 6, opacity: 0.7 }}>
          {fields.controllers.length}/{MAX_CONTROLLERS} controllers
          {onSize ? ` · ${variation.enabled ? variation.layers.length : 0}/${MAX_VARIATION_LAYERS} gradient` : ""}
        </span>
      </div>
      <div
        style={{
          display: "grid",
          gridTemplateColumns: `repeat(${kinds.length + (onSize ? 1 : 0)}, 1fr)`,
          gap: 4,
          marginBottom: 12,
        }}
      >
        {onSize && (
          <button
            ...
            disabled={gradientFull}
            aria-label="Add gradient layer"
            title={
              gradientFull
                ? `At most ${MAX_VARIATION_LAYERS} gradient layers`
                : variation.enabled
                  ? "Add a gradient layer over the whole sheet"
                  : "Add a gradient: one field over the whole sheet that scales every hole"
            }
            style={chip(false, {
              height: 42,
              display: "flex",
              ...
              justifyContent: "center",
              gap: 3,
              opacity: gradientFull ? 0.4 : 1,
              cursor: gradientFull ? "default" : "pointer",
            })}
          >
            <Waves size={13} />
            gradient
      ...
      {layerCount === 0 ? (
        <div style={hintStyle(theme)}>
          No {info.label.toLowerCase()} field yet.
          {onSize ? " A gradient varies the size across the whole sheet; a controller varies it where it reaches." : ""}
        </div>
      ) : (
        <>
          <div style={groupLabel}>{info.label} field layers</div>
              ...
              return (
                <div key={layer.id} style={{ display: "flex", gap: 4 }}>
                  <button
                    ...
                    aria-label={`Select gradient layer ${index + 1}`}
                    {...chipProps(active)}
                    style={rowChip(active, { opacity: layer.enabled ? 1 : 0.5 })}
                  >
                    <Waves size={11} style={{ flexShrink: 0 }} />
                    <span style={{ flex: 1, textAlign: "left" }}>
                      gradient · {layer.space.toLowerCase()} {layer.profile.toLowerCase()}
                      {layer.locked ? " · locked" : ""}
                    </span>
                    {layer.locked && <Lock size={9} style={{ flexShrink: 0, opacity: 0.7 }} />}
                  ...
                  <button
                    ...
                    aria-label={`Remove gradient layer ${index + 1}`}
                    title={
                      gradientLayers.length > 1
                        ? "Remove this gradient layer"
                        : "Remove the gradient (its settings are kept for the next one)"
                    }
              ...
              return (
                <div key={controller.id} style={{ display: "flex", gap: 4 }}>
                  <button
                    ...
                    {...chipProps(active)}
                    style={rowChip(active, { opacity: controller.enabled ? 1 : 0.5 })}
                  >
      ...
      {gradientSelected && selectedVariationLayer && (
        <div style={{ borderTop: `1px solid ${theme.sectionBorder}`, paddingTop: 12 }}>
          <div style={{ ...groupLabel, display: "flex", justifyContent: "space-between" }}>
            <span>
              Selected — gradient layer {variation.layers.indexOf(selectedVariationLayer) + 1}
              {variation.layers.length > 1 ? ` of ${variation.layers.length}` : ""}
            </span>
            <span style={{ color: theme.textFaint }}>whole sheet</span>
          </div>
          <GradientInspector />
        </div>
      )}
        ...
        <div style={{ borderTop: `1px solid ${theme.sectionBorder}`, paddingTop: 12 }}>
          ...
          {syncOptions.length > 0 && (
            <div style={{ marginBottom: 10 }}>
              <div style={{ fontSize: 10, color: theme.textSecondary, marginBottom: 5 }}>Follow the geometry of</div>
              <Select
