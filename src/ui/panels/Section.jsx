...
export function Section({ title, right, children, theme, last = false, collapsed = false }) {
  return (
    <div style={{ padding: 14, marginBottom: last ? 0 : 8, background: theme.cardBg, border: `1px solid ${theme.sectionBorder}`, borderRadius: 8 }}>
      <div style={{ fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1.2, color: theme.textSecondary, marginBottom: collapsed ? 0 : (right ? 10 : 14), fontFamily: MONO, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span>{title}</span>
        {right}
      </div>
      {!collapsed && children}
