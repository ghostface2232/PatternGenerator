...
export const RADIUS_MIN = 0.1,
  RADIUS_MAX = 2; // reach spread bounds
...
export function computeGizmo(layer, geom, minSep = 0) {
  ...
  const alongFrac = usesPosition ? clamp(layer.position ?? 0.5, 0, 1) : (((layer.phase || 0) % 1) + 1) % 1;
  let along = alongFrac * reachLen;
  if (minSep > 0 && along < minSep) along = minSep; // keep it grabbable near the origin
  const stopX = cx + dirX * along,
    stopY = cy + dirY * along;
  ...
  return {
    ...
    centerX: cx,
    centerY: cy,
    reachX: cx + dirX * reachLen,
    reachY: cy + dirY * reachLen,
    stopX,
    stopY,
