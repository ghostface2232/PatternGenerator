...
**Superellipse** (`geometry/superellipse.js`) is the morph shape: |x/a|ⁿ + |y/b|ⁿ = 1, with n = 1 a diamond, n = 2 an ellipse and n → ∞ a rectangle. It is the only shape a DOCUMENT can pick whose outline varies per hole, because the `shape` field channel drives its exponent — which is the reason every registry operation takes a trailing outline argument, and why `gap` reads `superN` off the hole objects. (The two shapes a layout imposes vary per hole too, and read that same argument; the other six ignore it.) Area is closed form (a Γ-function identity, not a polygon estimate, since OAR reads it) and the hit test is the implicit equation. Only `trace` and `svg` build a polygon, at `SUPER_SEGMENTS` vertices; that inscribed polygon is what an export writes, 0.16% under the exact area, which is below what the OAR readout resolves. `hole.cornerRadius` is meaningless here and is ignored.
...
- **Voronoi** sows sites with the Scatter sampler on the same `layout.scatter.seed` — the two modes are one point set drawn two ways, so switching between them keeps the arrangement — then builds each site's cell by clipping a convex polygon with one half-plane per nearby site and insets it by half the edge gap. No triangulation and no dependency. "Nearby" is proved rather than assumed: a cell is final only once every site within `reach` has been applied and no vertex of what is left is more than `reach/2` away, since a bisector's nearest point to the site is half the distance to it. Two neighbouring cells share an edge and each pulls back by g/2, so **the metal between them is exactly the edge gap** — asserted at 1.000000, 3.000000 and 8.000000 mm. Cells are scaled about their own CENTROID, not about their site: shrinking a hole must never move it closer to its neighbour, and scaling about any point inside the outline guarantees that, where the site can sit outside the inset cell as soon as a spacing field puts two sites closer together than the gap.
- **Flow Lines** integrates evenly spaced streamlines (Jobard and Lefebvre) of the base direction plus the angle channel, and cuts each as a slot whose width comes from the size channel **at every vertex of its own centreline**, so one slot narrows and widens across a gradient. Its width is capped at the radius the curve turns through, because a curve offset by more than that folds over itself and stops being a slot at all. It guarantees the same ligament Voronoi does, and two details are what make that true rather than nearly true. The proximity rule measures the STEP against the LINES, not the new point against sampled vertices of them — vertices sit half a step apart, so a point one separation from one of them can be 0.49 of one from the line between them, which pulled slots into each other under an angle controller. And seed candidates come out of a min-heap ordered by the separation the field asks for where each sits: taken in the order the lines offer them, a line starting in a sparse corner claims the ground a crowded region needed, and a controller asking for half the separation produced no extra lines at all.

**What the constant ligament is a property of.** Voronoi and Flow Lines both place their holes so that the edge gap is left between any two neighbours, everywhere. That is a statement about the PLACEMENT, at the hole size the document asks for. The size channel and the variation field then redraw those holes larger, and a spacing controller multiplies the pitch without touching the width — either one, pushed far enough, asks for holes that cannot fit beside each other, and they overlap. That is what those sliders mean in every mode here, the statistics report it (`minLigament` 0, every hole flagged), and a test pins that they do. What must never happen is the mode quietly ignoring what was asked for in order to keep a promise.

**Cross-hatch, the three free-form modes, Path, Voronoi and Flow Lines all REFUSE rather than truncate.** Each estimates its hole count up front — against `spacing.min`, since a 0.2× field multiplies it by five per direction — and returns an empty pattern when it exceeds its cap, and the Dimensions panel says why. None of them would give a coarser pattern by stopping at a cap: the area fills grow outward from the middle, so a cap leaves a disc of holes in an otherwise blank sheet, and Path would stop part-way along its first curve. Either reads as a broken pattern rather than as a limit. The grid family has no such cap and never has; the spacing channel does widen its worst case about fivefold (a 1000 mm panel of 0.5 mm holes at zero gap under a 0.2× field is twenty million holes), which is past the point where the rest of the pipeline is comfortable.
