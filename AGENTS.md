...
src/
  ...
  geometry/    hole shapes (SHAPES registry), superellipse, polygon helpers, boundary, ligament/overlap, OAR,
               spatial-hash.js (uniform-grid neighbour queries)
  layouts/     hole placement. index.js is the registry and the one entry point (generateHoles); grid.js
               ...
               spiral.js, fibonacci.js, lattice.js
...
doc (ui/useDocument.js reducer)
  ...
  → compileSpacing(doc.fields) the spacing channel, or null                        core/pipeline.js
  → generateHoles(params, sp)  hole centres (+ optional per-hole rotation `angle`) layouts/index.js
...
`core/document.js` → `createDocument()` is the schema (`schemaVersion` 3): `sheet`, `boundary` (margins, corner radius), `hole` (shape, sizes, corner radius, `shapeMix`), `layout` (type, gaps, `crosshatch` angles, `scatter` seed, radial block), `presetIndex`, `variation`, `fields` (the controller block), `assets` (image data for the image controllers), `taper`, `appearance`, `removedHoles`. UI-only state (theme, zoom, edit modes, the armed field tool, **which controller is selected**) lives in `App.jsx` and is never part of the document — selection in particular, because it changes on every click and each change would be an undo step that evicts a real edit from the hundred-step history.
...
**`spacing` is the one channel that moves a hole rather than redrawing it**, and everything unusual about it follows from that. It is compiled separately (`compileSpacing` in `core/pipeline.js`, not `compileDocumentField`) and handed to `generateHoles` as its second argument, because it is a sampler and `params` is a record of primitives. `compileSpacing` returns `{ sample, signature, min, max }` or `null`, and those four come from one call on purpose: `patternSignature` signs `signature` while the layouts read `sample`, so the two cannot describe different fields and leave a removed-hole index pointing at a hole that moved. `min`/`max` are rigorous bounds — the blend is a convex combination of the base value and the targets — which is what lets `scatter.js` size its neighbour grid without guessing. Not every mode reads the channel: see `layoutReadsSpacing` below.

**An image controller may not drive spacing** (`IMAGE_CHANNELS`). A brightness map is decoded from a bitmap by the DOM, asynchronously, and share links do not carry the picture at all — so a spacing controller reading one would make hole POSITIONS depend on state the document does not hold, and `removedHoles` would drift under a decode that no edit and no undo step accounts for. `compileControllers` drops the combination, and the rail and the panel do not offer it.
...
`generateHoles(params, spacing)` in `index.js` is the only entry point and holds the one destructuring of `params`; every mode below it takes explicit arguments. That is what lets `PLACEMENT_PARAMS` be *exactly* that destructuring, which `pipeline.test.js` asserts by parsing the function.
...
- **Scatter** is Bridson Poisson-disk sampling seeded from `layout.scatter.seed`. It guarantees `distance(i, j) ≥ min(rᵢ, rⱼ)` — `min`, not `max`, so a dense region packs right up to a sparse one instead of leaving a seam. With no spacing field every radius is equal and the minimum ligament is exactly the configured edge gap. It stops at `MAX_SCATTER_HOLES` (60 k); every other mode is closed-form and can afford the full slider range.
...
Within the grid family the channel varies the **row pitch only**, accumulated outward from the centre row and sampled at each row's own centre. Sampling a per-column pitch as well would vary the density in two dimensions but each row would sample the field at different points along its length, and the columns would stop lining up. Cross-hatch is the mode that varies both directions, and it does it by moving whole lines.
...
- OAR has two paths: theoretical (unit cell) for clean infinite patterns, counted (visible area sampling) whenever margins, corner radius, removal, variation, a live field channel, radial mode, or one of the free-form modes (Scatter, Spiral, Fibonacci — no unit cell to divide by) make the theoretical value wrong. Cross-hatch keeps a theoretical figure, but over the parallelogram its two line families cut, not over pitch × pitch. The two disagree slightly on the same geometry (35.4 vs 35.6 for the default document), so a change that only flips the path still moves the readout — which is why `activeFieldChannels` gates it rather than the raw controller count.
...
- **Nothing in the layouts may read `Math.random`, and `layouts.test.js` asserts every mode is a pure function of the document.** Scatter's randomness comes from `core/rng.js` (mulberry32) seeded by `layout.scatter.seed`. A layout that was not reproducible would break share links, exports and `removedHoles` at once.
- `geometry/spatial-hash.js` is the shared uniform grid: `SpatialHash` for "what is within r of this point" (the scatter sampler) and `forEachNeighbourPair` for the ligament and overlap walks. **Correctness never depends on the cell size** — every query rounds its radius up to whole cells and the caller does the exact distance test — so the cell size is a speed heuristic and nothing more. `calcMinLigament` sizes its grid from the hole extents, the layout's nominal spacing AND the holes' own mean neighbour distance; without the last term a scatter thinned by a spacing controller reported no ligament at all rather than a wide one.
