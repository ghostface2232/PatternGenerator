...
- **`MONO` names a metric-matched fallback, and it is load-bearing.** JetBrains Mono is fetched from Google Fonts at run time and is a tall face — ascent 1.020, descent 0.300, a 1.32 em line box against the 1.164 em of the monospace it replaces. At 11 px that is 1.7 px a line, so when it landed the sidebar used to drop 10 px in one frame, moving controls out from under the cursor. The `@font-face` in `GlobalStyles.jsx` gives the local monospace JetBrains Mono's vertical metrics, so both states occupy the same space and nothing moves; it downloads nothing (`local()` only) and falls through to the plain generic where metric overrides are unsupported. Drop either the face or the middle family in `MONO` and the jump comes back.
- **In the e2e suite, never press at a coordinate measured before an earlier step.** That 10 px reflow is what made `two quick slider gestures remain separate undo steps` fail in CI while passing everywhere the font is unreachable: the second press landed on empty panel, and a missed press on a slider is silent — the value simply does not change. Any future reflow does the same, since the control forgives only ~6 px of vertical error (a 12 px thumb overhanging a 4 px track). Click and hover through the locator (`locator.click({ position })`), which re-measures, waits for the box to hold still and verifies the point hits the control.
