...
상태 (2026-09-07): **구현과 자동 검증 완료, 실제 CAD 앱 수동 가져오기 검증은 미완료**. 공통 절단 윤곽·클리핑·커프(`export/profiles.js`), SVG 레이어·단위·절단 모드, DXF R2000(`export/dxf.js`), 내보내기 대화상자(`ui/ExportDialog.jsx`)를 원자적인 기능 커밋으로 구현했습니다. 문서 스키마는 변경하지 않았습니다. SVG/DXF는 같은 실제 절단 윤곽을 읽고, 원과 둥근 사각형·필은 가능하면 정확한 CAD 원호로 남깁니다. 절단 SVG 페이지와 DXF extents는 판 밖으로 확장되는 커프도 포함합니다.

검증: 전체 단위/브라우저 회귀 테스트, ezdxf 1.4.4로 70개 DXF 독립 감사(오류 0·수정 0)와 동반 SVG XML·단위 검사. 윤곽·기하, 파일 형식, UI 세 영역 리뷰와 별도 종합 리뷰를 수행했고, 큰 커프의 클리퍼 수치 오류·출력 범위 누락·모달 파일 드롭 문제를 수정했습니다. 실제 LibreCAD/Fusion/SolidWorks가 환경에 없어 해당 수동 검증을 완료했다고 표시하지 않습니다. 재현 스크립트와 수동 치수 확인 절차는 README의 Export verification에 기록했습니다.
...
상태 (2026-09-07): 1차 구현 완료. 아래 11.1에 진단과 결과를 적었고, 원래 계획은 11.2에 그대로 남겼습니다.

### 11.1 진단과 1차 재설계

Phase 2에서 5까지 기능은 갖춰졌지만, 그 기능들이 UI에 도착한 방식은 "각 Phase가 패널 하나를 더 붙인" 결과였습니다. 직접 써 보며 확인한 문제는 다음과 같습니다.

- 진입점이 흩어져 있었습니다. 캔버스 모드 다섯 개(크기 그라디언트, 컨트롤러, 경로, 경계, 홀 제거)가 각각 다른 패널 안의 "Edit on Canvas" 버튼이나 스위치로만 켜졌고, 그 패널을 스크롤해서 찾아야 했습니다. 모드가 서로 배타적이라는 사실도 배지 한 줄로만 드러났습니다.
- 컨트롤러와 경로는 핸들 하나씩만 끌 수 있었습니다. 선 컨트롤러를 옮기려면 양 끝점을 따로 끌어야 했고, 경로의 정점은 "가장 긴 구간의 중점"에만 추가할 수 있었으며, 클릭으로 커브를 그리는 펜이 없었습니다. Size Variation의 기즈모가 가진 Shift 스냅(45°, 사분위)은 다른 편집기에 옮겨지지 않았고, 컨트롤러와 경계는 Shift가 1 mm 격자에만 붙었습니다.
- 형상 편집기는 슬라이더 편집기였습니다. 레이어를 캔버스에서 잡아 옮기거나 돌릴 수 없었고, 불리언은 union과 subtract 둘뿐이었습니다.
- 이미지 컨트롤러는 밝기를 가중치로 읽었습니다. 검은 픽셀이 "영향 없음"이라 사진을 올려도 밝은 곳만 커졌고, 하프톤처럼 어두운 곳이 작은 홀이 되는 결과는 낼 수 없었습니다.
- 사이드바가 왼쪽, 도구가 없는 캔버스가 오른쪽에 있었고, 통계 카드가 캔버스 왼쪽 위를 가렸습니다. 섹션 열 개가 접히지 않는 한 스크롤로 이어져 있었습니다.

1차 재설계에서 바꾼 것은 다음과 같습니다. 문서 스키마는 7로 올렸습니다(이미지 컨트롤러의 읽기 모드).

- 배치: 왼쪽 모드 레일, 가운데 캔버스, 오른쪽 인스펙터. 레일은 Select(V), Boundary(B), Size gradient(G), Field controllers(F), Path(P, Path 레이아웃일 때), Remove(R)와 형상 편집기, 커맨드 팔레트(Ctrl+K)를 한 열에 둡니다. 레일에서 모드에 들어가면 그 모드를 소유한 인스펙터 섹션이 펼쳐져 스크롤됩니다. 패널의 "Edit on Canvas" 버튼은 그대로 두었습니다. 모드는 숫자가 있는 곳에서도 닿아야 하기 때문입니다. `actions.setMode` 하나가 다섯 모드의 유일한 스위치입니다.
- 캔버스 위 도구 레일은 모드에 따라 달라집니다. Fields에서는 채널과 컨트롤러 종류, Path에서는 펜과 정점 편집, Boundary에서는 컷아웃 추가, Remove에서는 전체 복원입니다.
- 통계는 캔버스 하단 상태 바로 내려갔고(홀 수, 최소 리거먼트, 면적, 줌 컨트롤), 좌상단에는 모드 배지와 경고만 남습니다. 배지는 모드의 색을 입고 그 모드에서 포인터가 하는 일을 한 줄로 말합니다.
- 인스펙터 섹션은 접히고(상태는 localStorage), Pattern, Fields, Manufacturing 세 그룹으로 묶였습니다. Size Variation은 Size Gradient로 이름을 바꿔 Field Controllers 바로 위에 두어, 전역 그라디언트와 국소 컨트롤러가 곱해지는 관계가 배치에서 읽히게 했습니다.
- 직접 조작: 컨트롤러, 경로 커브, 컷아웃은 본체를 끌어 통째로 옮기고, Shift는 축과 대각선에 잠깁니다. 정점을 Shift로 끌면 이웃 정점 기준 45° 배수에 잠기고 거리는 정수 mm에 붙습니다(geometry/snap.js, 컨트롤러·경로·경계·형상 편집기가 공유). 경로와 폴리라인은 커브 위를 더블클릭하면 그 자리에 정점이 생기고 정점을 더블클릭하면 사라집니다. 펜(P)은 클릭마다 정점을 덧붙이고, 선택된 커브가 없거나 닫혀 있으면 새 커브를 시작합니다. 3차 커브 컨트롤러는 앵커에서 제어점으로 이어지는 접선 팔을 그립니다. Delete는 선택된 컨트롤러, 컷아웃, 경로를 지웁니다. 커서는 핸들 위에서 pointer, 본체 위에서 move로 바뀝니다.
- 형상 편집기: 레이어를 캔버스에서 끌어 옮기고, 네 모서리 그립으로 중심 기준 크기 조절(Shift는 비율 유지), 위쪽 손잡이로 회전(Shift는 15° 단위), 다각형 정점 드래그와 더블클릭 추가·삭제. Pathfinder는 Union, Subtract, Intersect, Exclude 넷이 되었고(custom-shape.js의 composeLayers), 편집기 자체의 undo/redo, Delete, Ctrl+D 복제, 방향키 이동이 있습니다.
- 이미지 컨트롤러: 읽기 모드가 생겼습니다. halftone은 검은 픽셀이 `image.low`, 흰 픽셀이 `target`을 읽고 사각형 안에서 항상 완전한 가중치로 값을 정하므로 사진이 홀 크기의 하프톤이 됩니다. mask는 이전과 같은 읽기(밝기가 가중치)로, 기존 문서는 마이그레이션에서 이 모드를 받습니다. 인스펙터에는 전달 곡선 그래프, Halftone preset(size 채널에서 0.15배에서 1.6배), Fit to sheet가 있습니다.
- 단축키: V B G F P R, 1에서 4(채널), Esc(도구 다음 모드 해제), Delete, 0(뷰 리셋), +와 −(줌), Shift+H(오버레이), Ctrl+E(내보내기), Ctrl+K(팔레트), 기존 Ctrl+Z/Y/S. 텍스트 필드에 포커스가 있으면 글자 키는 글자입니다.
- 스타일: 토큰을 다듬고(theme.js의 채널·모드 색, 이징과 지속 시간 상수), 공용 컨트롤 스타일(controls/styles.js)로 패널들의 인라인 스타일 중복을 걷어냈습니다. 섹션 접힘, 배지 등장, 레일 활성 표시에 짧은 전환을 두고 prefers-reduced-motion을 존중합니다. 캔버스 바닥에는 줌을 따라가는 점 격자가 있습니다.

남긴 것: 반응형(1024px 미만 하단 시트), 터치 핀치 줌, 룰러와 스냅 그리드 표시, 컨트롤러 다중 선택, 형상 편집기의 정렬·분배 도구, 인스펙터 섹션 순서 사용자화. 커맨드 팔레트는 있으나 최근 사용 명령 기억은 없습니다.
...
| 3 | Phase 5 내보내기 | SVG 개선, DXF, 내보내기 대화상자 | 기능 커밋 3개 + 리뷰 수정 |
...
| 7 | Phase 6 성능 | 워커, WebGL 렌더러 | PR 4-5 |
| 8 | Phase 7 템플릿과 문서 | 갤러리, 온보딩, 가이드, i18n | PR 5-6 |

DXF를 Phase 2보다 앞에 두는 이유는 구현 비용이 낮고 현재 사용자에게 즉시 가치가 있기 때문입니다. UI 재설계는 별도 Phase가 아니라 Phase 0에서 뼈대를 만들고 Phase 2와 4에서 채웁니다. 실제로는 Phase 5 뒤에 11.1의 1차 재설계로 한 번에 정리했습니다.
