...
test("starting a new document saves the edits the outgoing one had not written yet", async ({ page }) => {
  ...
  // The slider is on the Boundary page and New on the Project page: one frame
  // between the rail's click and New lets React swap the page, and is still
  // far inside the debounce.
  await goTo(page, "boundary");
  const recent = await page.evaluate(async setSlider => {
    eval(setSlider)("Panel Width", 321);
    document.querySelector('button[aria-label="Project panel"]').click();
...
async function removeOneHole(page) {
  ...
  await goTo(page, "remove"); // opening the page enters removal
  await expect(page.getByRole("switch", { name: "Click to Remove" })).toHaveAttribute("aria-checked", "true");
